...
class UserController extends Controller {
	...
	public function login()
	{
		...
		else if($this->f3->exists('POST.login')) // OR $this->f3->VERB=='POST'
		{
			...
			{
				//jravia $this->f3->logger->write( "LOG IN: ".$this->f3->get('POST.username')." login failed (ip: " .$ip .")",'r'  );
				sleep(2);
			...
			{
				//jravia $this->f3->logger->write( "LOG IN: ".$this->f3->get('POST.username')." not activated (ip: " .$ip .")",'r'  );
				$this->f3->set('message',  $this->f3->get('i18n_not_activated'));
				$this->f3->set('page_head','Login');
				$this->f3->set('view','user/login.htm');
			}
			else 
			{
				...
				$this->f3->set('SESSION.user_id', $user->id);
				$user->login($user->id);
				//jravia $this->f3->logger->write( "LOG IN: ".$this->f3->get('POST.username')." login success (ip: " .$ip .")",'r'  );
				$this->f3->set('SESSION.logged_in', 'true');
				$this->f3->set('SESSION.timestamp', time());
				//echo 'path wrong ';exit;	
				$this->f3->reroute('/');
			}
		} 
		else
